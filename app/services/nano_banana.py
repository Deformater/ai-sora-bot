import json
import logging

import requests

from app.config.utils import get_settings

logger = logging.getLogger(__name__)

settings = get_settings()


class NanoBananaService:
    ENDPOINTS = {"kie": "https://api.kie.ai/api/v1/jobs/createTask"}
    CALLBACK_URL = f"{settings.CALLBACK_BASE_URL}/api/callback/nano-banana/kie"

    @classmethod
    def generate_image(
        cls,
        prompt,
        inputImage=None,
    ):
        if settings.API_SOURCE == "KIE":

            return cls.kie_image_generate(prompt, inputImage)

    @classmethod
    def kie_image_generate(
        cls,
        prompt,
        inputImage=None,
    ):
        payload = {
            "model": "google/nano-banana",
            "input": {"prompt": prompt, "output_format": "png", "image_size": "auto"},
            "callBackUrl": cls.CALLBACK_URL,
        }
        if inputImage:
            payload["input"]["image_urls"] = [inputImage]
            payload["model"] = "google/nano-banana-edit",

        headers = {
            "Content-Type": "application/json",
            "Accept": "application/json",
            "Authorization": f"Bearer {settings.KIEAI_API_KEY}",
        }

        response = requests.request(
            "POST", cls.ENDPOINTS["kie"], headers=headers, data=json.dumps(payload)
        )
        if response.status_code != 200:
            raise RuntimeError(response.text)

        logger.info(response.text)
        resp = response.json()
        return resp["data"]["taskId"]
